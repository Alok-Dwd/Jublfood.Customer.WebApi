...
{
    /// <summary>
    /// Common SQL Helper methods Implementation
    /// </summary>
    public static class SQLHelper
    {
        ...
        private static SqlConnection connection
        {
            get
            {
                //connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["ConnVC"].ConnectionString;
                ...
                //    if (connectionstring.Name.ToUpper() == "STOREDB")
                //    {
                //        connectionString = connectionstring.ConnectionString.ToString();
                //    }
                //    else
                //    {
                ...
                //    }
                //}
                connectionString = GlobalSetting.ConnectionString;
        ...
        static SQLHelper()
        {
            ...
            //        connectionString = string.Empty;
            //    }
            //return connectionstring["StoreDB"].ConnectionString.ToString();
            //}
            connectionString = GlobalSetting.ConnectionString;
            SqlConnection _connection = new SqlConnection(connectionString);
            //return _connection;
        ...
        private static void AddParameterToCommand(ref SqlCommand cmd, List<SqlParameter> lstParameters, int count)
        {
            cmd.Parameters.Add(lstParameters[count]);
            count++;
            if (lstParameters.Count > count) { AddParameterToCommand(ref cmd, lstParameters, count); }
        }

        public static DataTable SPExecuteDataTable(string spName, List<SqlParameter> lstSQLParameters)
        {
            #region Variables
            ...
            try
            {
                ...
                cmd = new SqlCommand() { CommandText = spName, CommandType = CommandType.StoredProcedure, Connection = connection };
                if (lstSQLParameters != null && lstSQLParameters.Count > 0)
                {
                    AddParameterToCommand(ref cmd, lstSQLParameters, 0);
                }
            ...
            finally
            {
                ...
                if (retTable != null) { retTable.Dispose(); }
            }
            return retTable;
        }

        public static int SPExecuteNonQuery(string spName, List<SqlParameter> lstSQLParameters)
        {
            #region Variables
            int retVal = 0;
            SqlCommand cmd = null;
            ...
            try
            {
                ...
                _con.Open();
                cmd = new SqlCommand() { CommandText = spName, CommandType = CommandType.StoredProcedure, Connection = _con };
                if (lstSQLParameters != null && lstSQLParameters.Count > 0)
                {
                    AddParameterToCommand(ref cmd, lstSQLParameters, 0);
                }
        ...
        public static int ExecuteNonQuery(string spName, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                SqlParameter[] commandParameters = SqlDataAccessParameterCache.GetSpParameterSet(connectionString, spName);
        ...
        public static int ExecuteNonQuery(SqlTransaction transaction, string spName, params object[] parameterValues)
        {
            ...
            {
                //pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
                SqlParameter[] commandParameters = SqlDataAccessParameterCache.GetSpParameterSet(transaction.Connection.ConnectionString, spName);
        ...
        /// <summary>
        /// Execute a stored procedure via a SqlCommand (that returns resultset) against the specified SqlConnection 
        /// using the provided parameter values.  This method will query the database to discover the parameters for the 
        /// stored procedure (the first time each stored procedure is called), and assign the values based on parameter order.
        /// </summary>
        /// <remarks>
        /// This method provides no access to output parameters.
        /// 
        /// e.g.:  
        ///  int result = ExecuteNonQuery(conn, "PublishOrders", 24, 36);
        /// </remarks>
        ...
        /// <param name="parameterValues">an array of objects to be assigned as the input values of the stored procedure</param>
        /// <returns>an int representing the number of rows affected by the command</returns>
        public static int ExecuteSPWithRerturnValue(string spName, params object[] parameterValues)
        {
            try
            {
        ...
        private static SqlDataReader ExecuteReader(SqlConnection connection, SqlTransaction transaction, CommandType commandType, string commandText, SqlParameter[] commandParameters, SqlConnectionOwnership connectionOwnership)
        {
            ...
            //connectionOwnership = SqlConnectionOwnership.Internal;
        ...
        #region Functions
        /// <summary>
        /// This Method used to reads the XML file whose path is passed as a parameter & 
        /// returns the 'value' of the 'key attribute' passed as a parameter.
        /// </summary>
        ///     <param>
        ///         <name="attributeName"/>
        ///         <Type = "string"/>
        ...
        /// <returns></returns>.
        /// </summary>
        /// <returns>String, Returns the 'value' of the 'Key' passed as a parameter</returns>
        private static string GetConfigurationAttribute(string attributeName, string configurationFilePath, string xPath)
        {
            ...
            try
            {
                if ((configurationFilePath != string.Empty))
                {
                    ...
                    myXmlDocument.Load(configurationFilePath);
                    XmlNode node = myXmlDocument.SelectSingleNode(xPath);
                    returnValue = node.Attributes.GetNamedItem(attributeName).InnerText;
                }
            }
            catch (Exception ex)
            {
    ...
    internal sealed class SqlDataAccessParameterCache
    {
